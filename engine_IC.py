...
def train(cfg):
    ...
    model = open_clip.model.DuDAG(cfg, embed_dim, vision_cfg, text_cfg, quick_gelu, cast_dtype=cast_dtype)
    ...
    for cur_epoch in range(start_epoch, 10):
        ...
        path = "./tmp/checkpoints/8/checkpoint_" + str(cur_epoch + 1) + ".pyth"
...
def test(cfg, load=None, mode = None):
    ...
    model = open_clip.model.DuGAD(cfg, embed_dim, vision_cfg, text_cfg, quick_gelu, cast_dtype=cast_dtype)
    model = model.cuda(device=device)
