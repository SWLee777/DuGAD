...
class AnomalyCLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details, classnames=None, state_list=None):
        ...
        self.prompts_pos=prompts_pos
        self.prompts_neg=prompts_neg
        ...
        embedding_pos_mean = torch.mean(embedding_pos, dim=0, keepdim=True)
        embedding_neg_mean = torch.mean(embedding_neg, dim=0, keepdim=True)
        
        tg = 0.5 * (embedding_pos_mean + embedding_neg_mean)
    ...
    def forward(self):
        ...
        return self.prompts_pos, self.prompts_neg
...
def get_prompts(clip_model, classnames, device="cuda", n_ctx=12, depth=9, t_n_ctx=4, state_list=None):
    ...
    with torch.no_grad():
        prompts_pos, prompts_neg = prompt_learner()
    
    return prompts_pos, prompts_neg

if __name__ == "__main__":
    parser = argparse.ArgumentParser("AnomalyCLIP", add_help=True)
    parser.add_argument("--depth", type=int, default=9, help="text embedding depth")
    parser.add_argument("--n_ctx", type=int, default=12, help="prompt length")
    parser.add_argument("--t_n_ctx", type=int, default=4, help="text embedding length")
    ...
    AnomalyCLIP_parameters = {"Prompt_length": args.n_ctx, 
    ...
    print("Generated positive prompts:")
    for i, prompt in enumerate(prompts_pos[:5]):
        print(f"{i+1}. {prompt}")
    
    print("\nGenerated negative prompts:")
    for i, prompt in enumerate(prompts_neg[:5]):
        print(f"{i+1}. {prompt}")